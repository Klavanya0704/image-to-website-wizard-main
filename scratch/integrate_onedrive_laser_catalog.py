...
with open(r"scratch\onedrive_laser_products_data.json", "r", encoding="utf-8") as f:
    onedrive_laser = json.load(f)

with open(r"scratch\catalog_84_data.json", "r", encoding="utf-8") as f:
    all_84 = json.load(f)

# Keep the other 5 categories (60 products total, 12 in each category)
other_categories_products = [
    p for p in all_84 if p["categorySlug"] not in ["3d-printing", "laser-cutting"]
]
...
# 2. Add all 23 OneDrive Laser Cutting products
for p in onedrive_laser:
    FULL_CATALOG.append({
        "id": p["id"],
        "name": p["title"],
        "title": p["title"],
        ...
        "created_at": "2026-08-24T00:00:00.000Z",
        "dimensions": "Precision Laser Dimension",
        "discount_price": p["price"],
        "lead_time": "1-2 Business Days",
        "material": "Natural Hardwood / Cast Acrylic / Birch Ply",
        "max_order_quantity": 30,
        "min_order_quantity": 1,
        "moq": 1,
        "rating_count": p["reviews"],
        "sku": f"IDEA-LC-{p['slug'].upper()[:8]}",
        "source": "AICTE IDEA Lab",
        "status": "in_stock",
        "subcategory": "laser-cutting",
        "tags": ["laser-cutting", "woodwork", "acrylic", "idea-lab"],
        "features": ["CO2 Precision Laser Cut", "AICTE IDEA Lab Certified", "Hand-Finished Edges"],
        "specs": { "Material": "Birch Ply / Cast Acrylic / Hardwood", "Origin": "AICTE IDEA Lab", "Process": "CO2 Laser Cutting & Engraving" }
    })

# 3. Add other 5 categories (60 products total)
...
with open(r"scratch\full_catalog_with_onedrive_laser.json", "w", encoding="utf-8") as f:
...
print(f"  - Laser Cutting: {len(onedrive_laser)}")
print(f"  - Other 5 categories: {len(other_categories_products)}")
...
export const CATEGORY_META = {
  ...
  "laser-cutting": {
    ...
    heroImage: "/products/v5/laser-cut-acrylic-monstera-leaf-led-night-lamp.jpg"
  },
  "cnc-machining": {
    name: "CNC Machining",
...
print("Updated src/lib/catalog.ts and src/lib/product-images.ts with authentic Laser Cutting products.")
