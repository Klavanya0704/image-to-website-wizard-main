...
CATEGORY_URL = f"{LIVE_URL}/category/3d-printing"
...
for idx, p in enumerate(products):
    ...
    try:
        ...
        print(f"[{idx+1:02d}/31] [MATCH: {'YES' if matched else 'NO'}] {p['title']:<50} | Local: {local_sha[:10]}... | Live: {live_sha[:10]}... ({len(live_bytes)} B)")
        results.append({
            "title": p["title"],
            "slug": p["slug"],
            "url": live_img_url,
        ...
        print(f"[{idx+1:02d}/31] [ERROR] {p['title']}: {e}")
        all_matched = False

print("=" * 100)
print(f"Total 3D Printing Products Audited: {len(results)} / {len(products)}")
print(f"All 31 Images Byte-for-Byte Match on Production: {'PASS (100%)' if all_matched else 'FAIL'}")
print("=" * 100)
