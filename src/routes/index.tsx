...
function Index() {
  const [ticketTheme, setTicketTheme] = useState<"modern" | "vintage">(() => {
    if (typeof window !== "undefined") {
      return (localStorage.getItem("student_ticket_theme") as "modern" | "vintage") || "vintage";
    }
    return "vintage";
  });

  const handleThemeChange = (theme: "modern" | "vintage") => {
    setTicketTheme(theme);
    if (typeof window !== "undefined") {
      localStorage.setItem("student_ticket_theme", theme);
    }
  };
  ...
  return (
    <div className="pb-16">
      ...
      {/* Student Discount Ticket Banner with Theme Toggle */}
      <section className="mx-auto max-w-[1400px] px-6 pt-6">
        {/* Theme Toggle Switcher Controls Header */}
        <div className="flex items-center justify-between mb-4 bg-muted/20 px-4 py-2.5 rounded-xl border border-border/45">
          <h3 className="text-xs font-black uppercase tracking-wider text-muted-foreground flex items-center gap-1.5">
            <Tag className="h-3.5 w-3.5 text-primary animate-pulse" /> Student Exclusive Offer
          </h3>
          <div className="inline-flex bg-muted dark:bg-card p-1 rounded-lg text-xs font-semibold select-none border border-border/20 shadow-inner">
            <button
              onClick={() => handleThemeChange("modern")}
              className={`px-3 py-1.5 rounded-md transition-all ${
                ticketTheme === "modern"
                  ? "bg-white dark:bg-muted text-foreground shadow-sm font-bold"
              ...
              <div className="flex-1 flex flex-col sm:flex-row items-start sm:items-center gap-6 z-10 pl-2">
                ...
                <div className="space-y-1.5">
                  ...
                  <p className="text-xs sm:text-sm text-emerald-100/80 font-medium max-w-2xl leading-relaxed">
                    Unlock exclusive discount on all 3D printing &amp; innovation gear. Verify
                    student ID at checkout.
                  </p>
                  <div className="pt-1">
                    <button
                      onClick={() => {
